...
class Rate_My_Post_Public {
  ...
  private function rating_widget_results_text( $options, $avg_rating = false, $vote_count = false, $post_id = false ) {
    ...
    if ( str_replace( ' ', '', $results_text ) ) { // custom results text exists
      $results_text = str_replace( '{{votecount}}','<span class="rmp-rating-widget__results__votes js-rmp-vote-count">( ' . $vote_count .' votos )</span>', $results_text );
      $results_text = str_replace( '{{avgrating}}','<span class="rmp-rating-widget__results__rating js-rmp-avg-rating">' . $avg_rating . '</span>', $results_text );
				...
				$results_text = $customization['rateResult'] . ' <span class="rmp-rating-widget__results__rating js-rmp-avg-rating">' . $avg_rating . '</span> / ' . $max_rating . '' . $customization['rateResult2'] . ' <span class="rmp-rating-widget__results__votes js-rmp-vote-count">( ' . $vote_count . ' votos )</span>';
		}
    return $results_text;
  }
